...
class AccessLink(object):
    ...
    def get_alertness(self, access_token):
        return self.oauth.get(endpoint="/users/sleepwise/alertness/", access_token=access_token)

    def get_bedtime(self, access_token):
        return self.oauth.get(endpoint="/users/sleepwise/circadian-bedtime/", access_token=access_token)

    def get_recharge(self, access_token):
        return self.oauth.get(endpoint="/users/nightly-recharge/", access_token=access_token)

    def get_continuous_heart_rate(self, access_token, date):
        return self.oauth.get(endpoint=f"/users/continuous-heart-rate/{date}", access_token=access_token)
